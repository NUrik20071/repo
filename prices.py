...
PRICE_DATA = {
    "KZ": {
        ...
        "products": {
            ...
            "diamonds": [
                {"id": "d50", "name": "💎 50+50 бонус", "price": 450, "bonus": True},
                {"id": "d150", "name": "💎 150+150 бонус", "price": 1370, "bonus": True},
                {"id": "d250", "name": "💎 250+250 бонус", "price": 2200, "bonus": True},
                {"id": "d500", "name": "💎 500+500 бонус", "price": 4350, "bonus": True},
            ...
            "stars": [
                {"id": "s50", "name": "⭐ 50 Stars", "price": 400},
                {"id": "s75", "name": "⭐ 75 Stars", "price": 610},
                {"id": "s100", "name": "⭐ 100 Stars", "price": 814},
                {"id": "s150", "name": "⭐ 150 Stars", "price": 1234},
                {"id": "s250", "name": "⭐ 250 Stars", "price": 2062},
                {"id": "s350", "name": "⭐ 350 Stars", "price": 2896},
                {"id": "s500", "name": "⭐ 500 Stars", "price": 4174},
                {"id": "s750", "name": "⭐ 750 Stars", "price": 6274},
                {"id": "s1000", "name": "⭐ 1000 Stars", "price": 8314},
                {"id": "s1500", "name": "⭐ 1500 Stars", "price": 12514},
            ],
        }
    },
    "RU": {
        ...
        "products": {
            ...
            "diamonds": [
                {"id": "d50", "name": "💎 50+50 бонус", "price": 90, "bonus": True},
                {"id": "d150", "name": "💎 150+150 бонус", "price": 265, "bonus": True},
                {"id": "d250", "name": "💎 250+250 бонус", "price": 430, "bonus": True},
                {"id": "d500", "name": "💎 500+500 бонус", "price": 880, "bonus": True},
            ...
            "stars": [
                {"id": "s50", "name": "⭐ 50 Stars", "price": 66},
                {"id": "s75", "name": "⭐ 75 Stars", "price": 101},
                {"id": "s100", "name": "⭐ 100 Stars", "price": 135},
                {"id": "s150", "name": "⭐ 150 Stars", "price": 205},
                {"id": "s250", "name": "⭐ 250 Stars", "price": 343},
                {"id": "s350", "name": "⭐ 350 Stars", "price": 482},
                {"id": "s500", "name": "⭐ 500 Stars", "price": 695},
                {"id": "s750", "name": "⭐ 750 Stars", "price": 1045},
                {"id": "s1000", "name": "⭐ 1000 Stars", "price": 1385},
                {"id": "s1500", "name": "⭐ 1500 Stars", "price": 2085},
            ],
        }
    }
}
...
class PriceDatabase:
    ...
    def init_database(self):
        ...
        # Migrations for databases created before display_order/name were added
        self._ensure_column(cursor, "products", "display_order", "INTEGER NOT NULL DEFAULT 0")
        self._ensure_column(cursor, "prices", "name", "TEXT")
    ...
    def populate_initial_data(self):
        ...
        # Build a global display order across all categories so the bot can
        # render items in the exact order they appear in PRICE_DATA.
        category_order = ["passes", "bundles", "diamonds", "stars"]
        display_order_by_id: Dict[str, int] = {}
        order_counter = 0
        # Use whichever region defines the catalog first; both regions share IDs.
        first_region = next(iter(PRICE_DATA.values()))
        for category in category_order:
            for product in first_region["products"].get(category, []):
                display_order_by_id[product["id"]] = order_counter
                order_counter += 1

        # Insert products and prices
        for region_code, region_data in PRICE_DATA.items():
            for category in category_order:
                for product in region_data["products"].get(category, []):
                    product_id = product["id"]
                    product_name = product["name"]
                    has_bonus = product.get("bonus", False)
                    price = product["price"]
                    display_order = display_order_by_id.get(product_id, 0)

                    # Upsert product so name/category/order fixes propagate on restart
                    cursor.execute("""
                        INSERT INTO products (id, category_id, name, has_bonus, display_order)
                        VALUES (?, ?, ?, ?, ?)
                    ...
                    """, (product_id, category, product_name, has_bonus, display_order))

                    # Upsert price (with per-region name) so changes propagate on restart
                    cursor.execute("""
                        INSERT INTO prices (product_id, region_id, name, price)
                        VALUES (?, ?, ?, ?)
                        ON CONFLICT(product_id, region_id) DO UPDATE SET
                            name = excluded.name,
                            price = excluded.price,
                            updated_at = CURRENT_TIMESTAMP
                    """, (product_id, region_code, product_name, price))
